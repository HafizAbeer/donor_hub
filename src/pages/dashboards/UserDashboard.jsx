...
export default function UserDashboard() {
  ...
  const userDonations =
    DUMMY_DONORS.find((d) => d.email === user?.email)?.donations || 5;
  ...
  const stats = [
    ...
    },
    {
      ...
      change: `+${Math.floor(availableDonors / 2)}`,
      icon: Users,
      color: "text-pink-600",
  ...
  const nearbyDonors = DUMMY_DONORS.filter((d) => d.email !== user?.email)
    .slice(0, 3)
    .map((donor, index) => ({
      name: donor.name,
      bloodGroup: donor.bloodGroup,
      distance: `${(index + 1) * 2.3} km`,
      lastDonation: getLastDonationText(donor.lastDonationDate),
    }));
