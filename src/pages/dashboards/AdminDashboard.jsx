...
import { Users, Heart, UserPlus, Activity, TrendingUp, Calendar, MapPin, Filter } from 'lucide-react';
...
const COLORS = ['#ef4444', '#ec4899', '#f43f5e', '#fb7185'];
...
export default function AdminDashboard() {
  ...
  const newThisMonth = DUMMY_DONORS.filter(d => d.lastDonation.includes('week') || d.lastDonation.includes('month')).length;
  const totalDonations = DUMMY_DONORS.reduce((sum, d) => sum + d.donations, 0);

  const stats = [
    { title: 'Total Donors', value: totalDonors.toString(), change: '+5', icon: Heart, color: 'text-red-600', bgColor: 'bg-red-100 dark:bg-red-900/30' },
    { title: 'Active Donors', value: activeDonors.toString(), change: `+${activeDonors}`, icon: Users, color: 'text-pink-600', bgColor: 'bg-pink-100 dark:bg-pink-900/30' },
    { title: 'New This Month', value: newThisMonth.toString(), change: `+${newThisMonth}`, icon: UserPlus, color: 'text-red-500', bgColor: 'bg-red-100 dark:bg-red-900/30' },
    { title: 'Total Donations', value: totalDonations.toString(), change: '+45', icon: Activity, color: 'text-pink-500', bgColor: 'bg-pink-100 dark:bg-pink-900/30' },
  ];
  ...
  const recentDonations = DUMMY_DONORS
    .sort((a, b) => {
      const aDays = a.lastDonation.includes('week') ? 7 : a.lastDonation.includes('month') ? parseInt(a.lastDonation) * 30 : 0;
      const bDays = b.lastDonation.includes('week') ? 7 : b.lastDonation.includes('month') ? parseInt(b.lastDonation) * 30 : 0;
      return aDays - bDays;
    })
    .slice(0, 5)
    .map((donor, index) => ({
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-red-900 dark:text-red-100 mb-2">Admin Dashboard</h2>
          <p className="text-red-700 dark:text-red-300 text-sm md:text-base">Manage donors and monitor donation activities</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <Card className="p-4 md:p-6 bg-white/80 dark:bg-red-950/50 backdrop-blur-sm border-red-200 dark:border-red-900">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg md:text-xl font-semibold text-red-900 dark:text-red-100">Monthly Trends</h3>
        ...
        <Card className="p-4 md:p-6 bg-white/80 dark:bg-red-950/50 backdrop-blur-sm border-red-200 dark:border-red-900">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={donationData}>
              ...
              <Line type="monotone" dataKey="newDonors" stroke="#ec4899" strokeWidth={2} name="New Donors" />
            </LineChart>
          </ResponsiveContainer>
        </Card>

        <Card className="p-4 md:p-6 bg-white/80 dark:bg-red-950/50 backdrop-blur-sm border-red-200 dark:border-red-900">
          <h3 className="text-lg md:text-xl font-semibold text-red-900 dark:text-red-100 mb-4">Recent Donations</h3>
            ...
            {recentDonations.map((donation) => (
              <div key={donation.id} className="flex items-center gap-3 p-3 bg-red-50 dark:bg-red-900/30 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-red-500 dark:text-red-500 mt-1">{donation.date}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="p-4 md:p-6 bg-gradient-to-br from-red-500 to-pink-500 text-white">
          ...
          <p className="text-sm opacity-90 mb-4">Register new donors to the platform</p>
